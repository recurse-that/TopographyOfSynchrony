\documentclass[letterpaper,11pt]{article}
%packages
\usepackage{amsfonts}
\usepackage{graphicx}
\usepackage[left=2cm,top=2cm,right=2cm,bottom=2cm,head=.5cm,foot=.5cm]{geometry}
\usepackage{url}
\usepackage{multirow}
\usepackage{longtable}
\usepackage{subfig}
\usepackage{float}
\usepackage{setspace}
\usepackage{lineno}
\usepackage{natbib}
\usepackage{amsmath}
\usepackage{xr}
\usepackage{authblk}

\graphicspath{{../Results}}
\def \kawFigPath 

%external documents
\externaldocument[MT-]{Paper}

%header material for paper
\title{Drivers of spatial synchrony in giant kelp populations}
\date{}

%***Tentative author order
\author{Lawrence W. Sheppard}
\author{Daniel C. Reuman}
\author{Tom Bell}
\author{Jonathan Walter}
\author{Kyle Cavanaugh}
\author{Max Castorani}

\begin{document}
\SweaveOpts{concordance=TRUE}

%The following is where you load in the numeric results that will be embedded in the text
<<eval=T,echo=F,message=F,warning=F>>=
#place R code here for loading in necessary variables from Results
@

\maketitle

%probably want table of contents, figure and table lists
\section*{Table Of Contents}
\newpage
\section*{Figures}
% Canyon Scene Synchrony Tables
\begin{figure}[htbp]
  \centering
  \textbf{Canyon Scenes} \\
  \vspace{5mm}
  \setkeys{Gin}{width=0.5\linewidth}
  \includegraphics{{Black Canyon of the Gunnison/tbl}}\,%
  \includegraphics{{Gunnison Point/tbl}}
  \includegraphics{{Marble Canyon/tbl}}\,%
\end{figure}
% Valley Scene Synchrony Tables
\begin{figure}[htbp]
  \centering
  \textbf{Valley Scenes} \\
  \vspace{5mm}
  \setkeys{Gin}{width=0.5\linewidth}
  \includegraphics{{Teays Valley/tbl}}\,%
  \includegraphics{{Oljato Monument Valley/tbl}}
\end{figure}
% Mesa and Plateau Scene Synchrony Tables
\begin{figure}[htbp]
  \centering
  \textbf{Mesa and Plateau Scenes} \\
  \vspace{5mm}
  \setkeys{Gin}{width=0.5\linewidth}
  \includegraphics{{Sentinel Mesa/tbl}}\,%
  \includegraphics{{Grand Mesa/tbl}}
\end{figure}
% Mountain Scene SynchronyTables
\begin{figure}[htbp]
  \centering
  \textbf{Mountain Scenes} \\
  \vspace{2mm}
  \setkeys{Gin}{width=0.5\linewidth}
  \includegraphics{{Mt Hood/tbl}}\,%
  \includegraphics{{Mt Rainier/tbl}}
  \includegraphics{{Mt St Helens/tbl}}\,%
  \includegraphics{{Mt Adams/tbl}}
  \includegraphics{{Mt McLoughlin/tbl}}\,%
  \includegraphics{{Mt Jefferson/tbl}}
\end{figure}

% Tiled Maps
% Canyon Scenes
\begin{figure}
  \includegraphics[width=\paperwidth]{{Black Canyon of the Gunnison/Scene_2D_maps/AllMapsTiled.pdf}}\,%
  \includegraphics[width=\paperwidth]{{Gunnison Point/Scene_2D_maps/AllMapsTiled.pdf}}
  \includegraphics[width=\paperwidth]{{Marble Canyon/Scene_2D_maps/AllMapsTiled.pdf}}
\end{figure}
% Valley Scenes
\begin{figure}[htbp]
  \includegraphics{{Teays Valley/Scene_2D_maps/AllMapsTiled.pdf}}
  \includegraphics{{Oljato Monument Valley/Scene_2D_maps/AllMapsTiled.pdf}}
\end{figure}
% Mesa and Plateau Scenes
\begin{figure}[htbp]
  \includegraphics{{Sentinel Mesa/Scene_2D_maps/AllMapsTiled.pdf}}
  \includegraphics{{Grand Mesa/Scene_2D_maps/AllMapsTiled.pdf}}
\end{figure}
% Mountain Scenes
\begin{figure}[htbp]
  \includegraphics{{Mt Hood/Scene_2D_maps/AllMapsTiled.pdf}}
  \includegraphics{{Mt Rainier/Scene_2D_maps/AllMapsTiled.pdf}}
  \includegraphics{{Mt St Helens/Scene_2D_maps/AllMapsTiled.pdf}}
  \includegraphics{{Mt Adams/Scene_2D_maps/AllMapsTiled.pdf}}
  \includegraphics{{Mt McLoughlin/Scene_2D_maps/AllMapsTiled.pdf}}
  \includegraphics{{Mt Jefferson/Scene_2D_maps/AllMapsTiled.pdf}}
\end{figure}

\end{document}