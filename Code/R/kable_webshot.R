...
kable_webshot <- function(scene_index) {

  table_png = paste(results_path,scene_names[scene_index],"table.png",sep='/')
  ...
  df %>%
    ...
    column_spec(2, color = ifelse(P_p_vals[1:4] < 0.3, "white", "black"),
                background = ifelse(P_p_vals[1:4] < 0.3, "blue", "white")) %>%
    column_spec(3, color = ifelse(P_p_vals < 0.3, "white", "black"),
                background = ifelse(P_p_vals < 0.3, ifelse(P_corr_vals<0, "red", "blue"), "white")) %>%
    column_spec(4, color = ifelse(S_p_vals[1:4] < 0.3, "white", "black"),
                background = ifelse(S_p_vals[1:4] < 0.3, "blue", "white")) %>%
    column_spec(5, color = ifelse(S_p_vals < 0.3, "white", "black"), 
                background = ifelse(S_p_vals < 0.3, ifelse(S_corr_vals<0, "red", "blue"), "white")) %>%
    save_kable(file = table_png, zoom=1.5)
  
  library(webshot)
  webshot(html_table_fname, table_png)
} # End function